...
from nsft_system_utils import touch_file
from nsft_shell_utils import call_shell_program, call_sh_module_fn
...
def test_call_shell_program_test_e_positive(dummy_tmp_file) -> None:
    out = call_shell_program("test", "-e", dummy_tmp_file)
    assert [] == out


def test_call_shell_program_test_e_positive(temp_dir) -> None:
    with pytest.raises(subprocess.CalledProcessError):
