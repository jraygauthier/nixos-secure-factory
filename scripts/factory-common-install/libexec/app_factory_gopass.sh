...
get_gopass_device_substore_key_pure_impl() {
  if ! [[ "x" == "${1:+x}" ]] || ! [[ "x" == "${2:+x}" ]]; then
    1>&2 echo "ERROR: get_gopass_device_substore_key_pure_impl: missing argument."
    return 1
  fi
...
get_gopass_device_substore_key_pure() {
  get_gopass_device_substore_key_pure_impl "$(get_gopass_device_vault_id)" "$@"
}


get_gopass_device_factory_only_substore_key_pure() {
  get_gopass_device_substore_key_pure_impl "$(get_gopass_factory_only_vault_id)" "$@"
}


get_gopass_device_substore_dir_pure() {
  local repo_dir_parent
  repo_dir_parent="$(get_gopass_device_vault_repo_dir | xargs -L 1 dirname)" || return 1

  local sskey
  sskey="$(get_gopass_device_substore_key_pure "$@")" || return 1

  echo "$repo_dir_parent/$sskey"
}


get_gopass_device_factory_only_substore_dir_pure() {
  local repo_dir_parent
  repo_dir_parent="$(get_gopass_factory_only_vault_repo_dir | xargs -L 1 dirname)" || return 1

  local sskey
  sskey="$(get_gopass_device_factory_only_substore_key_pure "$@")" || return 1
...
exists_gopass_device_substore() {
  local ssdir
  ssdir="$(get_gopass_device_substore_dir_pure "$@")" || return 1
...
exists_gopass_factory_only_device_substore() {
  local ssdir
  ssdir="$(get_gopass_device_factory_only_substore_dir_pure "$@")" || return 1
...
mount_gopass_device_substore() {
  ...
  print_title_lvl5 "Mounting device substore at '$sskey'"
...
umount_gopass_device_substore() {
  print_title_lvl5 "umount_gopass_device_substore"
...
mount_all_gopass_device_substores() {
  print_title_lvl3 "Mounting all device substores"
  ...
  while read -r dn; do
    print_title_lvl4 "Mounting device '$dn''s substore "
    mount_gopass_device_substore "$dn" || return 1
  done < <(echo "$device_names")
}


umount_all_gopass_device_substores() {
  print_title_lvl3 "Unmounting all device substores"
  # We assumes required gpg keys are already imported into the keyring.
  ...
  while read -r dn; do
    ...
    umount_gopass_device_substore "$dn" || return 1
  done < <(echo "$device_names")
}
...
deauthorize_factory_user_gpg_id_from_gopass_factory_vaults() {
  ...
  gpg_id_or_email="$(_process_gpg_id_input_from_either_args_or_clipboard "$@")" || return 1
  ...
  if ! $shallow; then
    ...
    while read -r dn; do
      ...
      device_substore_key="$(get_gopass_device_substore_key_pure "$dn")" || return 1
  ...
  deauthorize_factory_user_gpg_id_from_gopass_store "$factory_secrets_repo_root" "$matching_gpg_id"
  deauthorize_factory_user_gpg_id_from_gopass_store "$device_secrets_repo_root" "$matching_gpg_id"
...
authorize_factory_user_gpg_id_to_gopass_factory_vaults() {
  print_title_lvl1 "Authorizing factory user gpg id to gopass factory vaults"

  # --shallow flag. Allow to authorize only to top level stores.
  local shallow=false
  if printf "%s\n" "$@" | grep -q '\-\-shallow'; then
    shallow=true
  fi
  ...
  authorize_factory_user_gpg_id_to_gopass_store "$factory_secrets_repo_root" "$matching_gpg_id"
  authorize_factory_user_gpg_id_to_gopass_store "$device_secrets_repo_root" "$matching_gpg_id"
