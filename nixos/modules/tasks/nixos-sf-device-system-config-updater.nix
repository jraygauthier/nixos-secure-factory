...

{
  ...
  config = {
    systemd.services.nixos-sf-device-system-config-updater = {
      ...
      serviceConfig.SyslogIdentifier = "nixos-sf-device-system-config-updater";
      ...
      script = ''
        ...
        ${nixos-sf-device-system-config-updater}/bin/nixos-sf-device-system-config-update
      '';
    ...
    environment.etc."nixos-sf-device-system-config-updater/config-defaults.yaml" =
      let
        systemUpdaterEtcConfigDir = "nixos-sf-device-system-config-updater";
        ...
        systemUpdaterConfigDefaults = pkgs.stdenv.mkDerivation rec {
          version = "0.0.0";
          pname = "nixos-sf-device-system-config-updater-config-defaults";
          name = "${pname}-${version}";
