...
def test_export_and_import_secret_id(
    ...
    export_gpg_secret_keys_to_file(
        secret_keys_path, exp_ctx.gen_info.secret_keys[1].email,
        auth=exp_ctx.auth, proc=exp_ctx.proc)

    list_keys = list_gpg_keys_from_file(secret_keys_path, proc=imp_ctx.proc)
    assert 1 == len(list_keys)
    assert exp_email == list_keys[0].email

    import_gpg_key_file(secret_keys_path, proc=imp_ctx.proc)
...
def test_export_and_import_public_id(
    ...
    export_gpg_public_key_to_file(
        public_keys_path, exp_ctx.gen_info.secret_keys[1].email,
        auth=exp_ctx.auth, proc=exp_ctx.proc)
...
def test_export_and_import_secret_subkeys(
    ...
    export_gpg_secret_subkeys_to_file(
        subkeys_path, exp_ctx.gen_info.secret_keys[1].email,
        auth=exp_ctx.auth, proc=exp_ctx.proc)

    list_keys = list_gpg_keys_from_file(subkeys_path, proc=imp_ctx.proc)
    assert 1 == len(list_keys)
    assert exp_email == list_keys[0].email
